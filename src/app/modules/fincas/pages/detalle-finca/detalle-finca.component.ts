...
export class DetalleFincaComponent implements OnInit, OnDestroy{
  ...
  ngOnInit() {
    ...
    this.lotesService.updateLotes$.pipe(takeUntil(this.destroy$)).subscribe( data => {
      this.getLotes();
    });
  }
  ...
  private setFormulario( data ){
    ...
    this.formulario.controls.cadastral_record.setValue(data?.cadastral_record);
    this.formulario.controls.department_id.setValue(data?.department_id);
    this.formulario.controls.municipality_id.setValue(data?.municipality_id);
  ...
  changeDpto( event: any, municipio = null){
    ...
    this.parametricasService.getMunicipiosPorDpto(event)
      .subscribe( (data: any) => {
        ...
        if ( municipio ){
          const temp = this.parametricas.municipalities.find( e => e.id == municipio);
  ...
  submit(){
    if (this.formulario.invalid ) {
      ...
      this.alertService.activarLoading(true);
      this.fincasService.actualizarFinca(this.formulario.getRawValue())
        .subscribe( (data: any) => {
          console.log(data.data);
          
          this.detalleFinca.available_area = data.data.available_area;
          ...
          this.alertService.activarLoading(false);
          this.alertService.presentAlert('Se produjo un error en el registro de la finca, ' + error.error.message, ['Aceptar']);
        });
    }
  }
  ...
  openDetailLot(item){
    this.router.navigate([
      ...
      { id: item.id, name: item.name, areaDisponible: this.detalleFinca.available_area }
    ]);
  }
  ...
  async presentModal() {
    ...
    if (data.datos) {
      this.cargarDetalleFinca();
      this.detalleFinca.lotes.push({id: data.datos.id, name: data.datos.name});
    }
  }
  ...
  eliminar(){
    this.alertService.presentAlert(`¿Está seguro de eliminar la finca?`, [
      ...
      },
      {
        ...
        handler: () => {
          this.alertService.activarLoading(true);
          this.fincasService.eliminarFinca(this.id).subscribe( data => {
            this.alertService.activarLoading(false);
